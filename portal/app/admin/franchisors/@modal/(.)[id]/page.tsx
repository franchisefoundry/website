...
export default async function BrandModal({ params }: Props) {
  ...
  const who = (b.contact_name || b.brand_name || 'them').split(' ')[0]
  const M = messages ?? []
    ...
    <div className="space-y-4">
      <Section title="Snapshot">
        <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
          ...
          ].map(([k, v]) => <div key={k}><dt className="text-ink-3 mb-0.5">{k}</dt><dd className="font-medium text-ink capitalize">{v}</dd></div>)}
        </dl>
      </Section>
      <Section title={`Candidates matched (${candidates.length})`}>
        {candidates.length === 0 ? <p className="text-sm text-ink-3">None yet.</p> : (
          <div className="space-y-2">
            {candidates.map(m => {
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              const fe = m.franchisee_profiles as any
              return (
                <Link key={m.id} href={`/admin/franchisees/${fe?.id}`} className="flex items-center gap-3 rounded-xl border border-line-2 px-3.5 py-2.5 hover:bg-surface-2 transition-colors">
  ...
  const activity = (
    <div className="space-y-0">
      {[
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (b as any).answers_changed_at ? { t: 'Edited questionnaire answers', d: (b as any).answers_changed_at } : null,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (b as any).reviewed_at ? { t: 'Reviewed', d: (b as any).reviewed_at } : null,
        b.quiz_completed_at ? { t: 'Questionnaire submitted', d: b.quiz_completed_at } : null,
        { t: `Status: ${(b.status ?? '').replace('_', ' ')}`, d: b.updated_at },
        { t: 'Added to the network', d: b.created_at },
        ...
        return (
          <div key={i} className="relative pl-5 pb-4 last:pb-0">
            ...
            <p className="text-[13px] font-medium text-ink capitalize">{ev.t}</p>
  ...
  return (
    <DrawerHost expandHref={`/admin/franchisors/${id}`} ariaLabel={b.brand_name ?? undefined}>
      ...
      <RecordTabs tabs={[
        ...
        { value: 'messages', label: 'Messages', count: unread, panel: <ThreadPanel messages={M} threadType="franchisor" threadId={id} who={who} /> },
      ]} />
    </DrawerHost>
  )
}
