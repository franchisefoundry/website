...
export default async function FranchisorDetailPage({ params }: Props) {
  ...
  let agreementComments: { id: string; body: string; section_ref: string | null; created_at: string; author_name: string; resolved: boolean; admin_reply: string | null; admin_reply_at: string | null }[] = []
  if (franchisorAgreement?.id) {
    const { data: rawComments } = await admin
      .from('agreement_comments')
      .select('id, body, section_ref, created_at, author_id, resolved, admin_reply, admin_reply_at')
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    agreementComments = (rawComments ?? []).map((c: any) => ({ id: c.id, body: c.body, section_ref: c.section_ref, created_at: c.created_at, author_name: nameMap[c.author_id] ?? 'Brand', resolved: !!c.resolved, admin_reply: c.admin_reply ?? null, admin_reply_at: c.admin_reply_at ?? null }))
  }
