...
export function MessageThread({ messages, composer }: { messages: ThreadMessage[]; composer: React.ReactNode }) {
  return (
    <div className="bg-surface border border-line rounded-2xl shadow-[0_1px_2px_rgba(27,33,26,0.04)] flex flex-col overflow-hidden" style={{ minHeight: '60vh' }}>
      <div className="flex items-center gap-3 px-4 py-3 bg-surface-2 border-b border-line-2">
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold text-ink leading-tight">Franchise Foundry team</p>
          <p className="text-[11px] text-ink-3 flex items-center gap-1"><BellIcon className="w-3 h-3 flex-shrink-0" /> Your direct line — replies reach the team and push to their inbox</p>
        </div>
      </div>
